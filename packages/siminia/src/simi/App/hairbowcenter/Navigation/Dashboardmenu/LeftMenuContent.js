...
const CateTree = React.lazy(() => import('./CateTree'));
...
class LeftMenuContent extends React.Component{
    ...
    handelChangeTab = (type) => {
        ...
        $(`#navigation-${type}`).show();
        $(`.section-item-title`).removeClass('item-active')
        $(`#item-${type}`).addClass('item-active')
    }

    renderTabs() {
        return (
            <div className="section-title">
                <div id="item-menu" onClick={() => this.handelChangeTab('menu')} className={`section-item-title item-menu item-active`}>{Identify.__('Menu')}</div>
                <div id="item-account" onClick={() => this.handelChangeTab('account')} className={`section-item-title item-account`}>{Identify.__('Account')}</div>
                <div id="item-setting" onClick={() => this.handelChangeTab('setting')} className={`section-item-title item-settings`}>{Identify.__('Settings')}</div>
            </div>
        )
    }
        ...
        return (
            <div className="section-item-content">
                <Suspense fallback={<Loading />}>
                    <CateTree classes={classes} handleMenuItem={this.handleMenuItem.bind(this)} hideHeader={true} handleClose={this.parent.handleCloseMenu}/>
                    <Account handleMenuItem={this.handleLink.bind(this)} handleClose={this.parent.handleCloseMenu} isSignedIn={this.props.isSignedIn} />
    ...
    render(){
        const {classes} = this.props
        return (
            <div className={`${classes["list-menu-header"]} list-menu`} style={{maxHeight:window.innerHeight}}>
                {this.renderSections()}
